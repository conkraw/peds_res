...
def make_resident_names(n_residents: int) -> list[str]:
    ...
    for i in range(n_residents):
        ...
        names.append(st.sidebar.text_input(f"Resident {i + 1}", value=default, key=f"resident_{i}").strip() or default)
    return names
...
def make_shift_definitions() -> pd.DataFrame:
    ...
    shift_defs = st.sidebar.data_editor(
        ...
        column_config={
            ...
            "Hours": st.column_config.NumberColumn("Hours", min_value=0, step=0.5, required=True),
        },
...
def reconcile_schedule(
    ...
    merge_cols = ["Week", "Resident"]
    merged = blank[merge_cols].merge(existing[merge_cols + DAYS], on=merge_cols, how="left")
...
def summarize_schedule(
    ...
    long_df["Worked"] = long_df["Hours"] > 0
...
def make_excel(
    ...
    block_height = n_residents + 1  # one header row plus resident rows; keeps rows like 4,8,12...
    ...
    ws["A2"] = f"Generated {datetime.now().strftime('%Y-%m-%d %H:%M')}. Edit shift cells directly; formulas update in Excel."
    ...
    # Data validation for shift cells
    ...
    for week in range(1, num_weeks + 1):
        ...
        ws.cell(header_row, 1).value = f"Week {week}"
        ws.cell(header_row, 1).fill = week_fill
        ws.cell(header_row, 1).font = Font(bold=True)
        ws.cell(header_row, 1).alignment = Alignment(horizontal="center")
        ws.cell(header_row, 1).border = border
        ...
        for resident_idx, resident in enumerate(residents):
            ...
            # Weekly formulas at the right of each week row.
    ...
    for resident_idx, resident in enumerate(residents):
        ...
        for code_idx, _code in enumerate(shift_codes):
            ...
            summary_ws.cell(summary_row, col_idx).value = count_formula_for_rows(
                "Schedule", schedule_rows, criteria_ref
            )
        ...
        code_header_range = f"${get_column_letter(code_start_col)}${header_row}:${get_column_letter(code_end_col)}${header_row}"
        code_count_range = f"{get_column_letter(code_start_col)}{summary_row}:{get_column_letter(code_end_col)}{summary_row}"
    ...
    for col_idx in range(1, len(summary_headers) + 1):
        col_letter = get_column_letter(col_idx)
        summary_ws.column_dimensions[col_letter].width = max(12, min(22, len(str(summary_headers[col_idx - 1])) + 2))
...
def main() -> None:
    ...
    st.caption("Build D/N/OFF/POST schedules, count shifts, calculate hours, and export to Excel with formulas.")

    st.sidebar.header("Setup")
    num_residents = int(st.sidebar.number_input("Number of residents", min_value=1, max_value=20, value=3, step=1))
    num_weeks = int(st.sidebar.number_input("Number of weeks", min_value=1, max_value=26, value=5, step=1))
    ...
    if "schedule_df" not in st.session_state:
        st.session_state.schedule_df = build_blank_schedule(residents, num_weeks, default_code)

    schedule_df = reconcile_schedule(
    ...
    st.write("Edit the cells below. Use the dropdowns for D, N, OFF, POST, or any shift codes you add.")
    ...
    edited_schedule = st.data_editor(
        ...
        use_container_width=True,
        num_rows="fixed",
        disabled=["Week", "Resident"],
        column_config=column_config,
        key="schedule_editor",
    )

    for day in DAYS:
        edited_schedule[day] = edited_schedule[day].map(clean_code)

    st.session_state.schedule_df = edited_schedule

    st.subheader("Shift Counter")
    summary_df = summarize_schedule(edited_schedule, shift_defs, residents, num_weeks)
    st.dataframe(summary_df, use_container_width=True, hide_index=True)

    st.subheader("Export")
    excel_bytes = make_excel(edited_schedule, shift_defs, residents, num_weeks)
    ...
    with st.expander("What the Excel formulas do"):
        st.markdown(
            """
            The exported workbook has:

            - **Schedule** tab: editable schedule cells with dropdowns.
            - **Summary** tab: formulas that count each shift code across the repeated weekly rows.
            - **Shift_Definitions** tab: shift codes and hours.

            The summary formulas use the same logic as:

            `=SUM(COUNTIF($B$4:$H$4,"D"),COUNTIF($B$8:$H$8,"D"),COUNTIF($B$12:$H$12,"D"))`

            but they are generated automatically for the number of residents and weeks you select.
            """
        )
