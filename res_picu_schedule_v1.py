...
def make_excel(
    ...
    ws.freeze_panes = "B4"
    for col, width in {
        "A": 18, "B": 10, "C": 10, "D": 10, "E": 10, "F": 10, "G": 10,
        "H": 10, "I": 11, "J": 11, "K": 15, "L": 10, "M": 10, "N": 10,
    }.items():
...
def main() -> None:
    ...
    if st.session_state.get("schedule_signature") != signature:
        ...
        if "schedule_editor" in st.session_state:
            del st.session_state["schedule_editor"]

    st.subheader("Schedule")
    st.write("Edit the cells below. Dropdown choices should now stick on the first click.")
    ...
    st.data_editor(
        ...
        key="schedule_editor",
        on_change=sync_schedule_from_editor,
    )

    # Use the session-state copy, not the return value of data_editor.
    ...
    with st.expander("What changed in this version?"):
        st.markdown(
            """
            This version uses a small `on_change` callback for the schedule editor.
            That callback saves the dropdown value into `st.session_state.schedule_df`
            immediately, before Streamlit rerenders the page. That prevents the
            D/N/OFF/POST value from snapping back and needing a second click.
            """
        )
